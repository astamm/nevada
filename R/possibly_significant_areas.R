possibly_significant_areas_tot <-
  function(x,
    ...
    if (length(nonSign) > 0) {
      if (dimens > 1) {
        previously_excluded <-
          which(prodlim::row.match(data.frame(t(utils::combn(
            1:N, (dimens + 1)
          ))), data.frame(t(Y)), nomatch = 0) == 0)
        if (length(previously_excluded) > 0) {
          to_be_excluded <-
            subgroups(utils::combn(1:N, (dimens + 1))[, previously_excluded], dimens)
          eliminate1 <-
...
possibly_significant_areas_int <- function(x, y, Y, V, dimens, p_int, dimens1, representation = representation, distance = distance, alpha = alpha, N = N, B = B) {
  ...
  if (length(Z) == 0) {
    ...
    if (dim(nonSign)[2] > 0) {
      if (dimens > 1) {
        previously_excluded <- which(prodlim::row.match(data.frame(t(utils::combn(1:N, (dimens+1)))), data.frame(t(Y)), nomatch = 0)==0)
        if (length(previously_excluded)>0) {
          to_be_excluded <- subgroups(utils::combn(1:N, (dimens+1))[, previously_excluded], dimens)
    ...
    if (dimens <= length(dimens1)) {
      no_int <- utils::combn(dimens1, dimens)
...
possibly_significant_areas_ext <- function(x, y, Y, V, dimens, p_ext, representation = representation, distance = distance, alpha = alpha, N = N, B = B) {
  ...
  else
  {
    ...
    nonSign <- as.matrix(Y[,which(pvalue>=alpha)])
    if (dim(nonSign)[2] > 0) {
      #c <- cbind(utils::combn(Z,dimens),utils::combn(Y[,which(pvalue>=0.05)],dimens))
