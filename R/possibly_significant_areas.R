possibly_significant_areas_tot <- function(x, y, Y, V, dimens, p_tot, representation = representation, distance = distance, alpha = alpha, N = N, B = B) {
  ...
  for(j in 1:m) {
    ...
    for (k in 1:(dimens+1)) {
      for (l in k:(dimens+1)) {
        p_tot[Y[k,j], Y[l,j]] <- max(as.numeric(p_tot[Y[k,j], Y[l,j]]), pvalue[j])
      }
    }
  }
  ...
  if (length(Z) == 0) {
    ...
    if (dim(nonSign)[2] > 0) {
      ...
      if (dimens == 1) {
        ...
        #eliminate <- which(prodlim::row.match(data.frame(subgroups(Y[,which(pvalue<alpha)], dimens)), data.frame(subgroups(Y[,which(pvalue>=alpha)], dimens)), nomatch = 0)>0)
